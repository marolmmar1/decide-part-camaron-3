...
class QuestionOption(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.question.options.count() >= max_options and not (
            self.question.third_option and self.number == 3
        ):
            raise ValidationError(
                {
                    "options": [
        ...
        if (
            self.question.optionSiNo
            and self.option not in ["Sí", "No"]
            and self.number in [1, 2]
        ):
