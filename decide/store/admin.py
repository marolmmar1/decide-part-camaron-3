...
class VoteAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        votes = Vote.objects.values("voting_id").annotate(
            total_votes=Count("voting_id")
        )
        for vote in votes:
            census = (
                Census.objects.filter(voting_id=vote["voting_id"])
                .values("voter_id")
                .distinct()
                .count()
            )
            ...
            vote["start_date"] = (
                Voting.objects.filter(id=vote["voting_id"])
                .values("start_date")
                .first()["start_date"]
            )
            vote["end_date"] = (
